...
\begin{align*}
  p&=\gamma(t-h)=\gamma(t)-\gamma'(t)h+\frac{h^2}{2}\gamma''(t)\\
  r&=\gamma(t+h)=\gamma(t)+\gamma'(t)h+\frac{h^2}{2}\gamma''(t)
\end{align*}
と近似してみると、分母は2次までの近似を取って
\begin{align*}
  (q-p)\times(r-p)&=(-\gamma'(t)h+h^2\gamma''(t)/2)\times(\gamma'(t)h+h^2\gamma''(t)/2)\\
  &=-h^3\gamma'(t)\times\gamma''(t)
\end{align*}
...
これらを代入して
\[
  r=\frac{|p-q||q-r||r-p|}{2|(q-p)\times(r-p)|}=\frac{|\gamma'(t)|^3h^3}{|\gamma'(t)\times\gamma''(t)|h^3}=\frac{|\gamma'(t)|^3}{|\gamma'(t)\times\gamma''(t)|}
\]
と求められます。

もしも$\gamma$が既に弧長パラメータ付けされていれば、$|\gamma'(t)|=1$です。
ゆえに
\[
  r=\frac{1}{|\gamma'(t)\times\gamma''(t)|}
\]
...
\begin{lemma}
  ...
  このとき
  \[
    \left|\frac{d\gamma(s)}{ds}\times\frac{d^2\gamma(s)}{ds^2}\right|=\left|\frac{d^2\gamma(s)}{ds^2}\right|
  \]
\end{lemma}
\begin{proof}
  弧長パラメータにおいて、$d\gamma(s)/ds$と$d^2\gamma(s)/ds^2$は直交していたから、外積の幾何学的性質から
  \[
    \left|\frac{d\gamma(s)}{ds}\times\frac{d^2\gamma(s)}{ds^2}\right|=\left|\frac{d\gamma(s)}{ds}\right|\left|\frac{d^2\gamma(s)}{ds^2}\right|
  \]
  が成り立つ。
  ところが弧長パラメータにおいて$|d\gamma(s)/ds|=1$だったから、題意が従う。
\end{proof}
...
\begin{definition}
  ...
  \[
    \kappa(s):=\left|\frac{d^2\gamma(s)}{ds^2}\right|
  \]
\end{definition}
\noindent
意外とシンプルになりましたね。
...
弧長パラメータづけられた曲線$\gamma$において、$d^2\gamma(s)/ds^2$の長さには曲率という意味があることが明らかになりました。
そこで$d^2\gamma(s)/ds^2$を規格化したベクトルに名前を付けてやりましょう。
\begin{definition}
  $(I,\gamma)$を弧長パラメータ付け可能な曲線とし、$s=s(t)$をその弧長パラメータとする。
  このとき
  \[
    e_2(s):=\frac{d^2\gamma(s)/ds^2}{|d^2\gamma(s)/ds^2|}
  \]
...
\begin{definition}
  ...
  このとき、
  \[
    e_1(s):=\frac{d\gamma(s)}{ds}
  \]
  を\textbf{単位接ベクトル}という\footnote{
    弧長パラメータならすでに$|d\gamma(s)/ds|=1$なので、主法線ベクトルと違ってわざわざ規格化する必要はないです。
  }。
\end{definition}
...
\begin{definition}
  ...
  を\textbf{従法線ベクトル}という。
\end{definition}
曲線$(I,\gamma)$の曲がり具合はこれらのベクトル値関数$e_1,e_2,e_3$で記述できそうです。
ちなみに定義から明らかですが
\[
  e_i:s^{-1}(I)\to\mathbb{R}^3
\]
です。
...
\[
  \gamma(s):=(r\cos(s/r),r\sin(s/r),0)
\]
について考えてみます。
このとき
\begin{align*}
  \frac{d\gamma(s)}{ds}&=(-\sin(s/r),\cos(s/r),0)\\
  \frac{d^2\gamma(s)}{ds^2}&=-\frac1{r}(\cos(s/r),\sin(s/r),0)\\
\end{align*}
となります。このことから
\begin{align*}
...
となります。また曲率は
\[
  \kappa(s)=|d^2\gamma(s)/ds^2|=\frac1{r}
\]
となります。
...
$\gamma$は
\[
  |\gamma'(t)|=|(-r\sin(t),r\cos(t),a)|=\sqrt{r^2+a^2}\neq0
\]
...
\[
  \gamma(s)=\left(r\cos\left(\frac{s}{\sqrt{r^2+a^2}}\right),r\sin\left(\frac{s}{\sqrt{r^2+a^2}}\right),\frac{as}{\sqrt{r^2+a^2}}\right)
\]
となります。

このとき、単位接ベクトルは
\[
  e_1(s)=\frac{d\gamma(s)}{ds}=\frac1{\sqrt{r^2+a^2}}\left(-r\sin\left(\frac{s}{\sqrt{r^2+a^2}}\right),r\cos\left(\frac{s}{\sqrt{r^2+a^2}}\right),a\right)
\]
と求まります。
また、$e_2$を求めるために$\gamma$の二階微分を求めると
\[
  \frac{d^2\gamma(s)}{ds^2}=-\frac1{r^2+a^2}\left(r\cos\left(\frac{s}{\sqrt{r^2+a^2}}\right),r\sin\left(\frac{s}{\sqrt{r^2+a^2}}\right),0\right)
\]
なので、これの規格化は非常に簡単で
\[
...
曲率は
\[
  \kappa(s)=\left|\frac{d^2\gamma(s)}{ds^2}\right|=\frac{r}{r^2+a^2}
\]
となります。
...
まず単位接ベクトルを求めると
\[
  e_1(s)=\frac{d\gamma(s)}{ds}=\frac{d\gamma(t)}{dt}\frac{dt}{ds}=\frac1{ds/dt}\frac{d\gamma(t)}{dt}=\frac1{\sqrt{1+4t^2}}(1,2t,0)
\]
また、二階微分を求めると
\begin{align*}
  \frac{d^2\gamma(s)}{ds^2}&=\frac{d}{ds}e_1(s)\\
